...
export const getCafeList = async (
  ...
  try { 
    const response = await axios.get(
      `${baseUrl}/cafes?latitude=${lat}&longitude=${long}&page=${page}&limit=${limit}`,
    );
...
export const getRecommendations = async (
  ...
  try { 
    const response = await axios.get(
      `${baseUrl}/recommendations?latitude=${lat}&longitude=${long}&page=${page}&limit=${limit}`,
    );

    console.log(response.data);
    dispatch(setRecommendations(response.data));
  } catch (error) {
    console.log('Error getRecommendations api : ', error);
  } finally {
    console.log('getRecommendations finished');
  }
};
...
export const getCafeListBySearch = async (
  ...
  try {
    const response = await axios.get( 
      `${baseUrl}/search?query=${cafeName}&latitude=${currentLocation.latitude}&longitude=${currentLocation.longitude}&page=0&limit=10`,
    );
    console.log(response.data);
    dispatch(setCafeListBySearch(response.data));
  } catch (error) {
...
export const getCafeListBySelectedFilters = async (
  ...
  try {
    const response = await axios.post(`${baseUrl}/filterresult?latitude=${currentLocation.latitude}&longitude=${currentLocation.longitude}&page=${page}&limit=${limit}`, filters);
    console.log(response.data, 'getCafeListBySelectedFilters');
    dispatch(setCafeListBySelectedFilters(response.data));
  } catch (error) {
    console.log('Error getCafeListBySelectedFilters api : ', error);
  } finally {
    console.log('getCafeListBySelectedFilters finished');
  }
};
...
export const getRecommendedCafesByLocationFiltersSearches = async (
  ...
  try {
    ...
    // ];
    dispatch(setRecommendedCafesByLocationFiltersAndSearches(response.data));
  } catch (error) {
    console.log(
